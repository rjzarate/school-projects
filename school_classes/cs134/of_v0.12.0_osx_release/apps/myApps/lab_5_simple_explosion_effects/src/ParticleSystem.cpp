...
// Cyclic Force - force that rotates around the y around a position
//
CyclicForce::CyclicForce(float magnitude, ofVec3f *position) {
	this->magnitude = magnitude;
...
void CyclicForce::updateForce(Particle * particle) {
	...
	// Calculate the cross product, then multiply with magnitute
	ofVec3f p = *position - particle->position;
	ofVec3f dir = glm::cross((glm::vec3) p.getNormalized(), (glm::vec3) ofVec3f(0, 1, 0));

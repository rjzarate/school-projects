...
void Sprite::forward(float t) {
    ...
    // f(t) = o + dt
    pos += glm::rotate(headingVector, glm::radians(rot), glm::vec3(0, 0, 1)) * forwardSpeed * t / ofGetFrameRate();
}
...
void Sprite::rotate(glm::vec3 loc) {
    ...
    if (glm::abs(glm::abs(rot) - angle) < glm::abs(rotationSpeed)) {
        rot = angle;
        return;
    }
    
    if (shouldRotateLeft(rot, angle)) {
        rot -= rotationSpeed;
    } else {
        rot += rotationSpeed;
    }

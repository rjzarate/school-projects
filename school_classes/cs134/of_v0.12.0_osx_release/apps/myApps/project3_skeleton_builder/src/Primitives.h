...
public:
	Joint(glm::vec3 p, float r, ofColor diffuse = ofColor::lightGray, SceneObject* parent = NULL) {
		position = p;
		radius = r;
		...
		// Inc joint name counter
		counter++;
	}
	void draw();

	// Joint name automated
	static int counter;
	string jointName = "joint" + to_string(counter);

	// PARENT ALREADY EXISTS IN SCENE OBJECT
	// // When NULL, presumed parent
